...
def _get_bringup_launch_dir() -> Path | None:
    ...
    install_launch = PROJECT_ROOT / "install" / "bringup" / "share" / "bringup" / "launch"
...
def _discover_launch_map() -> dict[tuple[str, str, str], str]:
    ...
    files = sorted(launch_dir.glob("*.launch.py"), key=lambda p: (len(_stem(p)), _stem(p)))
    for path in files:
    ...
    _LAUNCH_MAP_CACHE = result
    # typer.echo(typer.style("[INFO] ", fg=typer.colors.BLUE) + f"Discovered {len(result)} launch file(s):")
...
def _complete_algorithm(ctx: typer.Context, args: list[str], incomplete: str) -> list[str]:
...
def _get_all_launch_stems() -> list[str]:
    ...
    if not launch_dir:
        return []
    stems = [p.name.removesuffix(".launch.py") for p in sorted(launch_dir.glob("*.launch.py"))]
    return stems
...
def _complete_launch_stem(ctx: typer.Context, args: list[str], incomplete: str) -> list[str]:
...
def _get_launch_command(adam_type: str, mocap_driver: str, algorithm: str) -> str:
    ...
    if key not in launch_map:
        echo_error(f"Unsupported combination: {adam_type} + {mocap_driver} + {algorithm}")
