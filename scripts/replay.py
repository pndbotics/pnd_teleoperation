...
def load_records(path: Path) -> list[tuple[int, bytes]]:
    ...
    with open(path, "rb") as f:
        ...
        if magic != MAGIC:
            raise ValueError(f"Bad magic in {path}: expected {MAGIC!r}, got {magic!r}")
        while True:
            ...
            if len(payload) != length:
                raise ValueError(f"Truncated payload in {path}: want {length}, got {len(payload)}")
            records.append((ts_ns, payload))
    return records


def main() -> int:
    parser = argparse.ArgumentParser(description="Replay a .udpcap recording to a UDP destination.")
    ...
    parser.add_argument("--port", type=int, default=12070, help="destination port (default: 12070)")
    parser.add_argument(
        "--speed",
        type=float,
        default=1.0,
        help="playback speed multiplier; 0 = topspeed (default: 1.0)",
    )
    parser.add_argument("--loop", type=int, default=1, help="loop count, 0 = infinite (default: 1)")
    parser.add_argument(
        "--src-port", type=int, default=0, help="bind source port (default: 0 = ephemeral)"
    )
    ...
    print(
        f"Loaded {len(records)} packets, {total_bytes / 1024:.1f} kB, "
