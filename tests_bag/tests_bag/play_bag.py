...
class BagPlayerForwarder(Node):
    def __init__(self):
        ...
        self.topic_map = {
            '/rosbag/temp/mocap/tf': '/rosbag/mocap/tf',
        }
        
        self.bag_publishers = {}
        
        # 启动 ros2 bag play 子进程
        self.bag_process = self.start_bag_play(self.get_parameter('bag_path').value)  # 替换为你的 bag 路径
    ...
    def start_bag_play(self, bag_path):
        cmd = ['ros2', 'bag', 'play', bag_path,
               '-l', 
               '--remap', '/mocap/tf:=/rosbag/temp/mocap/tf',
               ]
