...
class RetargetOptimization {
 public:
  RetargetOptimization(std::string config_json_path, bool jit = false);
  ...
  void setDefaultWeight(std::string name, double value){
    m_default_weights[name] = casadi::DM(value);
  }
 ...
 private:
  ...
  std::vector<casadi::DM> m_last_q_list, m_last_v_list;
