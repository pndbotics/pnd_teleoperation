...
class AdamRetarget : public rclcpp::Node {
 public:
  AdamRetarget() : Node("adam_retarget") {
    ...
    retarget_optimization = std::make_shared<RetargetOptimization>(config_json_path);
 ...
 private:
  ...
  void on_timer() {
    ...
    check_warm_start();
    // RCLCPP_INFO_THROTTLE(this->get_logger(), *this->get_clock(), 1000, "Got %d transforms", bonemap.size());

    // record solve time cost
    auto start = std::chrono::high_resolution_clock::now();
    auto retarget_params = this->retarget_optimization->calculate_params(bonemap);
    auto time_calculate_params = std::chrono::high_resolution_clock::now();
    ...
    auto duration_calculate_params =
        std::chrono::duration_cast<std::chrono::milliseconds>(time_calculate_params - start);
    auto duration_solve = std::chrono::duration_cast<std::chrono::milliseconds>(time_solve - time_calculate_params);
    auto duration_ext = std::chrono::duration_cast<std::chrono::milliseconds>(end - time_solve);

    RCLCPP_INFO_THROTTLE(this->get_logger(), *this->get_clock(), 1000,
                         "Solve time cost: %ld ms {f_param: %ld ms f_solve %ld ms f_ext %ld ms}", duration.count(),
                         duration_calculate_params.count(), duration_solve.count(), duration_ext.count());
