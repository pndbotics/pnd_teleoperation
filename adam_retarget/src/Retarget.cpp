...
RetargetOptimization::RetargetOptimization(std::string casadi_function_path, bool jit) {
  ...
  if (!retarget_config_file.is_open()) {
    std::cerr << "Error: Could not open file ref_io.json" << std::endl;
  } else {
  ...
  for (json::iterator it = opti_func_str_list.begin(); it != opti_func_str_list.end(); ++it) {
    m_opti_functions_list.push_back(casadi::Function::deserialize(it.value()));
  }
  ...
  for (json::iterator it = update_result_func_str_list.begin(); it != update_result_func_str_list.end(); ++it) {
    m_update_result_functions_list.push_back(casadi::Function::deserialize(it.value()));
  }

  auto post_process_function_str = retarget_config["extend_function"].get<std::string>();
  m_post_process_function = casadi::Function::deserialize(post_process_function_str);

  auto params_cal_function_str = retarget_config["params_calculate_function"].get<std::string>();
  m_cal_params_function = casadi::Function::deserialize(params_cal_function_str);
