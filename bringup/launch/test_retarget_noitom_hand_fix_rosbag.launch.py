...
def generate_launch_description():
    ...
    urdf_name = "urdf/adam_standard_plus/urdf_plus/standard_plus53.urdf"
    ...
    static_transform_publisher_node = Node(
        ...
        remappings=[("/tf", "/mocap_nots/tf"), ("/tf_static", "/mocap_nots/tf_static")],
    )
    ...
    flushbag_timestamp = Node(
        package="adam_retarget",
        executable="remove_timestamp.py",
        name="remove_timestamp",
        parameters=[{"topicfrom": "/mocap/tf", "topicto": "/mocap_nots/tf"}],
    )
    ...
    ld.add_action(flushbag_timestamp)
