...
class SimpleHTTPSServer:
    ...
    async def start(self):
        ...
        try:
            ...
            print(
                f"🌐 HTTPS server started on {self.config.host_ip}:{self.config.https_port}"
            )
...
class VRMonitor:
    ...
    def __init__(self, vr_src_path: Optional[str] = None):
        self.config = None
        self.vr_server = None
        self.https_server = None
        self.is_running = False
        self.goal = None  # Store the latest goal (contains all controller data)
        self._goal_lock = threading.Lock()  # Thread lock for goal access
        if vr_src_path:
            global XLEVR_PATH
            XLEVR_PATH = vr_src_path
            print(f"Set XLEVR_PATH to: {XLEVR_PATH}")
    ...
    def initialize(self):
        ...
        self.config.enable_https = (
            True  # Enable HTTPS server, VR requires web interface
        )
    ...
    async def monitor_commands(self):
        ...
        while self.is_running:
            try:
                ...
                print(
                    "⚠️  No VR command received in the last 3 seconds, still waiting..."
                )
                continue
            except Exception as e:
    ...
    def get_latest_goal_nowait(self, arm=None):
        ...
        Args:
            arm: If specified ("left" or "right"), return that arm's goal.
                 If None, return the latest goal (which contains all controller data).
        """
        with self._goal_lock:
            return self.goal
    ...
    def get_left_goal_nowait(self):
        ...
        if goal and "LeftHand" in goal:
            return goal["LeftHand"]
        return None
    ...
    def get_right_goal_nowait(self):
        ...
        """
        goal = self.get_latest_goal_nowait()
        if goal and "RightHand" in goal:
            return goal["RightHand"]
        return None
