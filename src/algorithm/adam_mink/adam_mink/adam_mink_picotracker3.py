...
class AdamMinkPicoTracker3Node(AdamMinkBase):
    """Pico Tracker 3-based Adam robot IK node with joystick control."""

    def __init__(self, node_name: str = "adam_mink_picotracker3") -> None:
        """Initialize the Pico Tracker 3 node."""
        super().__init__(node_name)
        self.calibrated = False
        qos = QoSProfile(durability=DurabilityPolicy.TRANSIENT_LOCAL, depth=1)
        self.sub_calibrated = self.create_subscription(
            Bool, "pico_mocap/calibrated", self.calibrated_callback, qos
        )
        ...
        self.get_logger().info("Adam Mink Pico Tracker 3 IK node initialized.")

    def calibrated_callback(self, msg: Bool) -> None:
        """Callback for calibrated topic."""
        self.calibrated = msg.data
        self.get_logger().info(f"Calibrated: {self.calibrated}")
...
def main(args=None) -> None:
    """Main entry point for the Pico Tracker 3 node."""
    rclpy.init(args=args)
    print(f"Starting Adam Mink Pico Tracker 3 IK node... {args}")

    node = AdamMinkPicoTracker3Node()
