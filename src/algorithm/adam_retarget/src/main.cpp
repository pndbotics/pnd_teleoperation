...
class AdamRetarget : public rclcpp::Node {
 public:
  AdamRetarget() : Node("adam_retarget") {
    ...
    #ifdef __aarch64__
    RCLCPP_INFO(this->get_logger(), "Current platform: aarch64");
    {
      const std::string from = "opti_config";
      const std::string to = "opti_config/aarch64";
      const auto pos = config_json_path.find(from);
      if (pos != std::string::npos) {
        config_json_path.replace(pos, from.size(), to);
      }
    }
    #else
    RCLCPP_INFO(this->get_logger(), "Current platform: x86_64");
    #endif
